            ...
            return (
              <div className={`mt-12 ${modulasRes !== 0 && 'ml-auto'} max-w-[895px]`} key={index}>
                ...
                <div className="mt-9 px-8 pt-19 pb-14 rounded-[100px] bg-black-50 text-light-text">
                  {item.contents.map((content, i) => <p key={`${item.title} - ${i}`} className="text-2xl text-light-text mb-10 leading-12">{content}</p> )}
                  <div className="mt-6">
                  {item.options.map((option, i) => {
                    const {listItems, contents} = option;
                    return (
                    <>
                      <h3 className="mt-20 pb-3 max-w-[740px] text-center mx-auto border border-b-5 border-t-0 border-r-black border-l-black border-b-item-heading-border rounded-br-[100px]  rounded-bl-[100px]" key={option.title + '-' + i}>{option.title}</h3>
